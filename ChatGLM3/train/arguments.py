...
class ModelArguments:
    ...
    lora_alpha: Optional[float] = field(
        ...
        metadata={
            "help": (
                "A higher value results in more significant adjustments, potentially improving adaptation to used tasks or data, "
...
class DataTrainingArguments:
    ...
    train_data: Optional[str] = field(
        default=None, metadata={"help": "The input train author data file (a jsonlines or csv file)."}
    )
    ...
    train_format: str = field(
        default=None, metadata={"help": "The format of the training data file (multi-turn or input-output)"},
    )
...
class GLMTrainingArguments(TrainingArguments):
    ...
    warmup_ratio: float = field(
        default=0.1,
        metadata={
            "help": "ratio of training steps to perform linear learning rate warmup for (not applicable to all schedulers)"},
    )
