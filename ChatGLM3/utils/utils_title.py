...
class INDDataSet(Dataset):
    ...
    def __init__(self, dataset, tokenizer, max_source_length, max_target_length):
        ...
        self.instruct = ("Identify the outlier text from the collection based on these rules:\n Here is a set of paper titles: \n ### {} \n ### Does the title ### {} ### belong to the primary group of these papers? Provide an answer of either ‘yes’ or ‘no’.")
    ...
    def __getitem__(self, index):
        ...
        title = title if len(self.tokenizer.tokenize(title)) < 200 else ' '.join(
            title.split(' ')[:100])  # limit the disambiguate paper title token lenth
